...
mastersRouter.post("/users", (req, res) => {
  const { username, name, password, role, branch_id, department_id, PF_NO } =
    req.body;
...
mastersRouter.put("/users/:id", (req, res) => {
  const { username, name, role, branch_id, department_id, PF_NO, password } =
    req.body;
  const password_hash = bcrypt.hashSync(password, 10);
  const user = {
    username,
...
mastersRouter.delete("/users/:id", (req, res) => {
  ...
  const date = new Date();
  const resignedDate = date.toISOString().slice(0, 10); 
...
mastersRouter.put("/branches/:id", (req, res) => {
  const { code, name } = req.body;
  pool.query(
    "UPDATE branches SET code = ?, name = ? WHERE id = ?",
...
mastersRouter.get("/transfers", (req, res) => {
  pool.query(
    `SELECT e.id, u.name,b1.name as old_branch,b2.name as new_branch  FROM employee_transfer e 
              join users u on e.staff_id=u.id join branches b1 on b1.id=e.old_branch_id 
              join branches b2 on b2.id=e.new_branch_id`,
    (error, results) => {
      if (error)
        return res.status(500).json({ error: "Internal server error" });
      res.json(results);
...
mastersRouter.put("/updateentries/:id", (req, res) => {
  ...
  pool.query(updateQuery, [kpi, account_no, value, date, new Date(), req.params.id], (error) => {
    if (error) {
...
mastersRouter.post("/update_employee_transfer", (req, res) => {
  ...
  pool.query(
    ...
    (err, rows) => {
      if (err) return res.status(500).json({ error: "Database error 1", details: err });

      if (rows.length === 0) {
        return res.status(404).json({ error: "No resigned allocations found for this user" });
      }
      ...
      pool.query(
        ...
        [period, branchId, userId],
        (err, result) => {
          if (err) return res.status(500).json({ error: "Database error 2", details: err });

          if (result.length === 0) {
            return res.status(404).json({
              error: "No employee_transfer record found for this user. Update not possible.",
            });
          }
          ...
          pool.query(
            ...
            (err) => {
              if (err)
                return res.status(500).json({ error: "Database error 3", details: err });
              ...
              pool.query(
                ...
                (err) => {
                  if (err)
                    return res.status(500).json({ error: "Database error 4", details: err });

                  return res.json({
                    success: true,
                    message: "Transfer updated and user branch cleared successfully",
